...
class _CalendarScreenState extends State<CalendarScreen> {
  ...
  _updateLinkedHashMap() {
    ...
    for (Medicine med in allMeds) {
      ...
      switch (med.interval) {
        case 'once':
          if(map[med.startDate] == null) map[med.startDate] = [];
          map[med.startDate].add(med);
          break;
        case 'daily':
          ...
          while (temp.isBefore(med.endDate)) {
            ...
            //
            // }
            // temp.add(Duration(hours: med.intervalTime));
            // med.startTime.add(Duration(hours: med.intervalTime));
            // n--;
          }
          break;
        case 'weekly':
          ...
          while (temp.isAfter(med.endDate)) {
            Medicine m = Medicine.fromMapString(med.toMapString());
            m.startTime = temp;
            if(map[med.startDate] == null) map[med.startDate] = [];
            map[med.startDate].add(med);
